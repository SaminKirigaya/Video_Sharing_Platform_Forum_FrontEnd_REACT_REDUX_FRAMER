...
function SearchBasedOnTag() {
    ...
    const showAllOldVideosInCard = ()=>{
      ...
      return currentData.map((each, index)=>{
        ...
        <div className="card-body" style={{backgroundColor: '#c0ff1d'}}>
          ...
          <p class="card-text smollTitle normalLine">{makeItSmoll(each.title)}</p>
  ...
  return (
    ...
    <div className='container-fluid pages flex-column'>
        ...
        <motion.h5 animate={{opacity: [0,1]}} transition={{duration:1.5}} className='mx-auto text-center mt-4 mb-4 welcomeTxt headLine'>Category - "{searchText[0].toLocaleUpperCase()+searchText.slice(1,searchText.length)}" ...</motion.h5>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className='row row-cols-1 row-cols-md-4 mb-5 d-flex justify-content-start p-3' ref={scope}>
          ...
          {oldVideos ? showAllOldVideosInCard() : <div className='d-flex mx-auto w-100 justify-content-center'><p className='mx-auto fontcol normalLine'>No such video matches according to your Category ... 😖</p></div>}
          {oldVideos.length<1 ? <div className='d-flex mx-auto w-100 justify-content-center'><p className='mx-auto fontcol normalLine'>Some Error Occured Or It Seems The Server Has No Video Uploaded According To Your Category ... 😖</p></div>:null}
...
export default SearchBasedOnTag
